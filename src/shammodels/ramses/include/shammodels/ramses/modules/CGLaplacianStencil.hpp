...
namespace shammodels::basegodunov {
    ...
    inline T laplacian_7pt(
        ...
        /*
                ...
                T laplace_x
                    = (W_i - W_xm)
                        * (1.0 / ((lev_diff_xm > 0) ? dx * 0.75 : ((lev_diff_xm < 0) ? dx * 1.5 :
           dx)))
                    + (W_xp - W_i)
                            * (1.0 / ((lev_diff_xp > 0) ? dx * 0.75 : ((lev_diff_xp < 0) ? dx * 1.5
           : dx)));

                T laplace_y
                    = (W_i - W_ym)
                        * (1.0 / ((lev_diff_ym > 0) ? dy * 0.75 : ((lev_diff_ym < 0) ? dy * 1.5 :
           dy)))
                    + (W_yp - W_i)
                            * (1.0 / ((lev_diff_yp > 0) ? dy * 0.75 : ((lev_diff_yp < 0) ? dy * 1.5
           : dy)));

                T laplace_z
                    = (W_i - W_zm)
                        * (1.0 / ((lev_diff_zm > 0) ? dz * 0.75 : ((lev_diff_zm < 0) ? dz * 1.5 :
           dz)))
                    + (W_zp - W_i)
                            * (1.0 / ((lev_diff_zp > 0) ? dz * 0.75 : ((lev_diff_zp < 0) ? dz * 1.5
           : dz)));
